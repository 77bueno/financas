...
const backBtnStyle: React.CSSProperties = {
  background: 'var(--w6)', border: 'none', width: 34, height: 34, borderRadius: 11,
...
const rowInputStyle: React.CSSProperties = {
  display: 'flex', alignItems: 'center', gap: 10, background: 'var(--w4)',
  borderRadius: 14, padding: '8px 12px',
};

function StepTag({ label }: { label: string }) {
  return <span style={{ fontFamily: "'Inter'", fontSize: 11.5, color: 'var(--green)', fontWeight: 600, letterSpacing: '.08em' }}>{label}</span>;
}
    ...
    <div className="onboard-wrap" style={{ display: 'flex', flexDirection: 'column', minHeight: 680, animation: 'fadeIn .3s both' }}>
      {state.onbStep === 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, gap: 22, paddingTop: 30 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <h1 style={{ margin: 0, fontFamily: "'Space Grotesk'", fontSize: 30, fontWeight: 700, color: 'var(--t1)', lineHeight: 1.15, letterSpacing: '-.02em' }}>
              Todo o seu dinheiro,<br />num lugar só.
            </h1>
            <p style={{ margin: 0, fontSize: 14.5, color: 'var(--t3)', lineHeight: 1.55 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <button
              ...
              style={{
                width: '100%', padding: 16, background: 'var(--w6)', border: '1px solid var(--w12)',
                borderRadius: 16, color: 'var(--t1)', fontSize: 15, fontWeight: 600, cursor: 'pointer', fontFamily: "'Inter'",
              }}
            >
            ...
            <button
              ...
              style={{ width: '100%', padding: 10, background: 'none', border: 'none', color: 'var(--t4)', fontSize: 13, cursor: 'pointer', fontFamily: "'Inter'" }}
            >
              Pular por agora
            </button>
          </div>
      ...
      {state.onbStep === 1 && (
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, gap: 18, paddingTop: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <h2 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: 'var(--t1)' }}>Como podemos te chamar?</h2>
            <p style={{ margin: 0, fontSize: 13.5, color: 'var(--t3)' }}>Só pra personalizar o seu painel.</p>
          </div>
          <input
            ...
            autoFocus
            style={{
              width: '100%', background: 'var(--w5)', border: '1px solid var(--w12)',
              borderRadius: 16, padding: '16px 18px', color: 'var(--t1)', fontFamily: "'Inter'", fontSize: 17,
              outline: 'none',
            }}
          />
          <div style={{ flex: 1 }} />
      ...
      {state.onbStep === 2 && (
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, gap: 18, paddingTop: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <h2 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: 'var(--t1)' }}>Onde você guarda dinheiro?</h2>
            <p style={{ margin: 0, fontSize: 13.5, color: 'var(--t3)' }}>Adicione suas contas e o saldo atual de cada uma. Você pode editar tudo depois.</p>
          </div>
              ...
              <div key={a.id} style={rowInputStyle}>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 3 }}>
                  ...
                  <input
                    ...
                    inputMode="numeric"
                    placeholder="0"
                    style={{ width: 72, background: 'transparent', border: 'none', outline: 'none', color: 'var(--green)', fontFamily: "'Space Grotesk'", fontSize: 15, fontWeight: 600, textAlign: 'right' }}
                  />
                </div>
                <button onClick={() => actions.removeAccount(a.id)} style={{ background: 'var(--w6)', border: 'none', color: 'var(--t4)', width: 26, height: 26, borderRadius: 8, fontSize: 13, cursor: 'pointer', flexShrink: 0 }}>✕</button>
              </div>
            ))}
            <button
      ...
      {state.onbStep === 3 && (
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, gap: 18, paddingTop: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <h2 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: 'var(--t1)' }}>Quanto você recebe por mês?</h2>
            <p style={{ margin: 0, fontSize: 13.5, color: 'var(--t3)' }}>Usamos pra planejar quanto sobra pra investir.</p>
          </div>
          <div style={{ background: 'linear-gradient(150deg, rgba(52,211,153,.16), rgba(16,185,129,.08))', border: '1px solid var(--w10)', borderRadius: 22, padding: 22 }}>
      ...
      {state.onbStep === 4 && (
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, alignItems: 'center', justifyContent: 'center', gap: 20, textAlign: 'center', padding: '40px 10px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <h2 style={{ margin: 0, fontSize: 24, fontWeight: 700, color: 'var(--t1)' }}>Tudo pronto{state.userName.trim() ? `, ${state.userName.trim()}` : ''}!</h2>
            <p style={{ margin: 0, fontSize: 14, color: 'var(--t3)', lineHeight: 1.55 }}>
              Seu painel está montado com o que você adicionou. Registre gastos, investimentos e metas pelo botão <strong style={{ color: 'var(--green)' }}>+</strong>.
            </p>
          </div>
          <button
            onClick={actions.finishOnb}
      ...
      {state.connecting && (
        ...
        >
          ...
          <span style={{ fontSize: 14, color: 'var(--t2)' }}>Preparando dados de exemplo…</span>
        </div>
      )}
    </div>
  );

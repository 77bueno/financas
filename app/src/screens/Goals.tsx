    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, animation: 'fadeIn .3s both' }}>
      ...
      {derived.hasGoals && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'rgba(142,123,255,.12)', border: '1px solid rgba(142,123,255,.25)', borderRadius: 18, padding: 15 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, color: '#C9C5DE' }}>Total guardado</div>
            <div style={{ fontFamily: "'Space Grotesk'", fontSize: 17, color: '#fff', fontWeight: 600 }}>{derived.goalsSavedStr}</div>
          </div>
          <span style={{ fontSize: 12, color: '#9C97B8' }}>de {derived.goalsTargetStr}</span>
        </div>
      )}
      ...
      {derived.hasGoals ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, padding: '32px 20px', textAlign: 'center' }}>
          <div style={{ width: 70, height: 70, borderRadius: 22, background: 'rgba(110,231,176,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32 }}>🐷</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <strong style={{ fontSize: 16, color: '#F3F1FF' }}>Nenhum objetivo ainda</strong>
            <span style={{ fontSize: 13, color: '#9C97B8', lineHeight: 1.5 }}>Crie cofrinhos pra guardar dinheiro com propósito: uma viagem, reserva de emergência, um celular novo…</span>
          </div>
        </div>
      )}

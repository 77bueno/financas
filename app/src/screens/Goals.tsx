  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, animation: 'fadeIn .3s both' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
        <span style={{ fontSize: 12, color: '#9AA3AF', letterSpacing: '.04em' }}>COFRINHOS & METAS</span>
        <strong style={{ fontSize: 22, color: '#EDEFF2', fontWeight: 700 }}>Meus objetivos</strong>
      </div>
      ...
      {derived.hasGoals && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'rgba(16,185,129,.12)', border: '1px solid rgba(16,185,129,.25)', borderRadius: 18, padding: 15 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, color: '#C3C9D2' }}>Total guardado</div>
            <div style={{ fontFamily: "'Space Grotesk'", fontSize: 17, color: '#fff', fontWeight: 600 }}>{derived.goalsSavedStr}</div>
          </div>
          <span style={{ fontSize: 12, color: '#9AA3AF' }}>de {derived.goalsTargetStr}</span>
        </div>
      )}
      ...
      {derived.hasGoals ? (
        <div className="grid-2">
          {derived.goalRows.map(g => (
            <button
              ...
              style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.07)', borderRadius: 20, padding: 18, display: 'flex', flexDirection: 'column', gap: 13, cursor: 'pointer', fontFamily: "'Inter'", textAlign: 'left', width: '100%' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14.5, color: '#EDEFF2', fontWeight: 600 }}>{g.name}</div>
                  <div style={{ fontSize: 11.5, color: '#6B7280' }}>{g.sub}</div>
                </div>
                <span style={{ fontFamily: "'Space Grotesk'", fontSize: 13, color: '#34D399', fontWeight: 600 }}>{g.pctStr}</span>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12.5, width: '100%' }}>
                <span style={{ color: '#C3C9D2', fontFamily: "'Space Grotesk'" }}>{g.savedStr}</span>
                <span style={{ color: '#6B7280' }}>de {g.targetStr}</span>
              </div>
            </button>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, padding: '32px 20px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <strong style={{ fontSize: 16, color: '#EDEFF2' }}>Nenhum objetivo ainda</strong>
            <span style={{ fontSize: 13, color: '#9AA3AF', lineHeight: 1.5 }}>Crie cofrinhos pra guardar dinheiro com propósito: uma viagem, reserva de emergência, um celular novo…</span>
          </div>
        </div>
      )}

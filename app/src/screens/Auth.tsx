...
const label: React.CSSProperties = { fontSize: 12, color: '#9AA3AF', fontWeight: 500, display: 'block', marginBottom: 6 };

const inputWrap: React.CSSProperties = {
  display: 'flex', alignItems: 'center', gap: 10,
  background: '#14171C', border: '1px solid rgba(255,255,255,.1)', borderRadius: 10,
  padding: '0 12px', marginBottom: 14,
};

const inputStyle: React.CSSProperties = {
  flex: 1, minWidth: 0, background: 'transparent', border: 'none', outline: 'none',
  color: '#EDEFF2', fontFamily: "'Inter'", fontSize: 14, padding: '12px 0',
};
...
const linkBtn: React.CSSProperties = {
  background: 'none', border: 'none', color: '#34D399', fontSize: 13, fontWeight: 500,
...
export function Auth() {
  ...
  if (recoveryCode) {
    return (
      <div style={{ minHeight: '100dvh', background: '#0B0D10', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: "'Inter'" }}>
        <div style={{ width: '100%', maxWidth: 420, background: '#101318', border: '1px solid rgba(255,255,255,.08)', borderRadius: 16, padding: 28 }}>
          <div style={{ width: 40, height: 40, borderRadius: 10, background: 'rgba(52,211,153,.14)', color: '#34D399', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>
            <Icon name="key" size={20} />
          </div>
          <h2 style={{ margin: '0 0 8px', fontSize: 19, color: '#EDEFF2', fontWeight: 700 }}>Guarde seu código de recuperação</h2>
          <p style={{ margin: '0 0 18px', fontSize: 13.5, color: '#9AA3AF', lineHeight: 1.55 }}>
            É a <strong style={{ color: '#EDEFF2' }}>única forma</strong> de redefinir sua senha se você esquecer — seus dados ficam só neste dispositivo e não há e-mail de recuperação. Anote em lugar seguro:
          </p>
          <div style={{ background: '#14171C', border: '1px dashed rgba(52,211,153,.4)', borderRadius: 10, padding: '16px 12px', textAlign: 'center', marginBottom: 18 }}>
            <span style={{ fontFamily: "'Space Grotesk'", fontSize: 22, fontWeight: 600, color: '#34D399', letterSpacing: '.06em' }}>{recoveryCode}</span>
          </div>
    ...
    <div style={{ minHeight: '100dvh', background: '#0B0D10', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: "'Inter'" }}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 11, marginBottom: 28, justifyContent: 'center' }}>
          ...
          <strong style={{ fontSize: 20, color: '#EDEFF2', fontWeight: 700, letterSpacing: '-.01em' }}>Finanças</strong>
        </div>

        <div style={{ background: '#101318', border: '1px solid rgba(255,255,255,.08)', borderRadius: 16, padding: 28 }}>
          <h1 style={{ margin: '0 0 4px', fontSize: 20, color: '#EDEFF2', fontWeight: 700 }}>{titles[mode][0]}</h1>
          <p style={{ margin: '0 0 20px', fontSize: 13, color: '#9AA3AF', lineHeight: 1.5 }}>{titles[mode][1]}</p>
          ...
          <Field icon="lock">
            ...
            />
            <button onClick={() => setShowPass(s => !s)} style={{ background: 'none', border: 'none', color: '#6B7280', cursor: 'pointer', padding: 4, display: 'flex' }}>
          ...
          {error && (
            <div style={{ background: 'rgba(248,113,113,.1)', border: '1px solid rgba(248,113,113,.3)', borderRadius: 10, padding: '10px 12px', marginBottom: 14, fontSize: 12.5, color: '#FCA5A5' }}>
              {error}
            </div>
          )}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 18 }}>
            {mode === 'login' ? (
              <>
                ...
                <button onClick={() => switchMode('recover')} style={{ ...linkBtn, color: '#9AA3AF' }}>Esqueci a senha</button>
              </>
            ) : (
        ...
        <p style={{ textAlign: 'center', fontSize: 11.5, color: '#5B6472', marginTop: 18, lineHeight: 1.5 }}>
          Suas contas e dados ficam salvos <strong style={{ color: '#8B93A0' }}>somente neste dispositivo</strong>.<br />

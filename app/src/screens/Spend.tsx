    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, animation: 'fadeIn .3s both' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          <strong style={{ fontSize: 22, color: '#EDEFF2', fontWeight: 700 }}>{title}</strong>
        </div>
        <button onClick={actions.goTxns} style={{ background: 'none', border: 'none', fontSize: 12.5, color: '#34D399', cursor: 'pointer', fontFamily: "'Inter'" }}>ver extrato ›</button>
      </div>
      ...
      {derived.totalSpend > 0 ? (
          ...
          {derived.spendCats.map(c => (
            <div key={c.name} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 11, background: 'rgba(255,255,255,.04)', borderRadius: 16 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 11, color: '#6B7280' }}>{c.pct}% dos gastos</div>
              </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, padding: '40px 20px', textAlign: 'center' }}>
          <div style={{ width: 70, height: 70, borderRadius: 22, background: 'rgba(248,113,113,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32 }}>🧾</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <strong style={{ fontSize: 16, color: '#EDEFF2' }}>Nenhum gasto registrado</strong>
            <span style={{ fontSize: 13, color: '#9AA3AF', lineHeight: 1.5 }}>Registre seus gastos e este relatório mostra pra onde seu dinheiro está indo, por categoria.</span>
          </div>
          <button
            onClick={actions.openAdd}
            style={{ padding: '13px 22px', background: 'linear-gradient(135deg,#10B981,#059669)', border: 'none', borderRadius: 14, color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: "'Inter'" }}
          >
            + Registrar um gasto
          </button>
        </div>

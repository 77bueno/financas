...
const chip = (active: boolean, color = '#8E7BFF'): React.CSSProperties => ({
  flexShrink: 0, padding: '7px 13px', borderRadius: 99, fontSize: 12.5, fontWeight: 500,
  cursor: 'pointer', fontFamily: "'Sora'",
  background: active ? 'rgba(142,123,255,.22)' : 'rgba(255,255,255,.04)',
  border: `1px solid ${active ? color : 'rgba(255,255,255,.08)'}`,
  color: active ? '#fff' : '#C9C5DE',
});
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, animation: 'fadeIn .3s both' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
          <span style={{ fontSize: 12, color: '#9C97B8', letterSpacing: '.04em' }}>EXTRATO</span>
          <strong style={{ fontSize: 20, color: '#F3F1FF', fontWeight: 700 }}>Todas as transações</strong>
        </div>
        <button
          onClick={actions.openAdd}
          style={{ background: 'linear-gradient(135deg,#8E7BFF,#5E3EE0)', border: 'none', height: 36, padding: '0 14px', borderRadius: 12, color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: "'Sora'" }}
        >
          + Nova
        </button>
      </div>
      ...
      <input
        ...
        style={{
          ...
          borderRadius: 14, padding: '12px 15px', color: '#fff', fontFamily: "'Sora'", fontSize: 13.5, outline: 'none',
        }}
      />
      ...
      <div className="fin-scroll" style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 }}>
        ...
        <button style={chip(typeFilter === 'despesas', '#FF8FB3')} onClick={() => setTypeFilter('despesas')}>Despesas</button>
        <button style={chip(typeFilter === 'receitas', '#6EE7B0')} onClick={() => setTypeFilter('receitas')}>Receitas</button>
        {usedCats.map(c => (
      ...
      {groups.length === 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, padding: '40px 20px', textAlign: 'center' }}>
          <div style={{ width: 64, height: 64, borderRadius: 20, background: 'rgba(142,123,255,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28 }}>🧾</div>
          <span style={{ fontSize: 13.5, color: '#9C97B8', lineHeight: 1.5 }}>
            {derived.txnRows.length === 0 ? 'Nenhuma transação registrada ainda.' : 'Nada encontrado com esses filtros.'}
          </span>
          {derived.txnRows.length === 0 && (
            <button
              onClick={actions.openAdd}
              style={{ padding: '12px 20px', background: 'linear-gradient(135deg,#8E7BFF,#5E3EE0)', border: 'none', borderRadius: 13, color: '#fff', fontSize: 13.5, fontWeight: 600, cursor: 'pointer', fontFamily: "'Sora'" }}
            >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {groups.map(([label, txs]) => (
          <div key={label} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <span style={{ fontSize: 12, color: '#9C97B8', letterSpacing: '.04em', padding: '2px 2px 0' }}>{label.toUpperCase()}</span>
            {txs.map(t => (
              <button
                key={t.id}
                ...
                style={{
                  ...
                  border: '1px solid rgba(255,255,255,.06)', borderRadius: 16, cursor: 'pointer', fontFamily: "'Sora'", textAlign: 'left',
                }}
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13.5, color: '#F3F1FF', fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.desc}</div>
                  <div style={{ fontSize: 11.5, color: '#7C7896' }}>{t.cat}</div>
                </div>
                <span style={{ fontFamily: "'Space Grotesk'", fontSize: 14, fontWeight: 600, color: t.color, flexShrink: 0 }}>{t.amountStr}</span>
                <span style={{ color: '#7C7896', fontSize: 15, flexShrink: 0 }}>›</span>
              </button>
            ))}
          </div>
        ))}
